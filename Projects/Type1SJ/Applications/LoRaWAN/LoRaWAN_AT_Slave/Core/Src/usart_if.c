...
void vcom_Trace(uint8_t *p_data, uint16_t size)
{
  ...
  HAL_UART_Transmit(&husart1, p_data, size, 1000);
...
UTIL_ADV_TRACE_Status_t vcom_Trace_DMA(uint8_t *p_data, uint16_t size)
{
  ...
  HAL_UART_Transmit_DMA(&husart1, p_data, size);
  return UTIL_ADV_TRACE_OK;
...
UTIL_ADV_TRACE_Status_t vcom_ReceiveInit(void (*RxCb)(uint8_t *rxChar, uint16_t size, uint8_t error))
{
  ...
  while (__HAL_UART_GET_FLAG(&husart1, USART_ISR_BUSY) == SET);

  /* Make sure that UART is ready to receive)   */
  while (__HAL_UART_GET_FLAG(&husart1, USART_ISR_REACK) == RESET);

  /* Enable USART interrupt */
  __HAL_UART_ENABLE_IT(&husart1, UART_IT_WUF);

  /*Enable wakeup from stop mode*/
  HAL_UARTEx_EnableStopMode(&husart1);

  /*Start USART1 receive on IT*/
  HAL_UART_Receive_IT(&husart1, &charRx, 1);
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *husart1)
{
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *husart1)
{
  /* USER CODE BEGIN HAL_UART_RxCpltCallback_1 */

  /* USER CODE END HAL_UART_RxCpltCallback_1 */
  if ((NULL != RxCpltCallback) && (HAL_UART_ERROR_NONE == husart1->ErrorCode))
  {
